...
use tools::custom::{CustomTools, GetLatestAlertParams, ProposeWazuhRuleParams};
...
impl WazuhToolsServer {
    fn new() -> Result<Self, anyhow::Error> {
        ...
        let api_port: u16 = env::var("WAZUH_API_PORT")
            .unwrap_or_else(|_| "55000".to_string()).parse().unwrap_or(55000);
        ...
        let indexer_port: u16 = env::var("WAZUH_INDEXER_PORT")
            .unwrap_or_else(|_| "9200".to_string()).parse().unwrap_or(9200);
        ...
        let vulnerability_client_arc = Arc::new(tokio::sync::Mutex::new(wazuh_vulnerability_client));
        ...
        let agent_tools         = AgentTools::new(agents_client_arc.clone(), vulnerability_client_arc.clone());
        let alert_tools         = AlertTools::new(indexer_client_arc.clone());
        let custom_tools        = CustomTools::new(indexer_client_arc.clone());
...
impl WazuhToolsServer {
    ...
    async fn get_wazuh_latest_alert(
        &self,
        Parameters(params): Parameters<GetLatestAlertParams>,
    ) -> Result<CallToolResult, McpError> {
        self.custom_tools.get_wazuh_latest_alert(params).await
    }
    ...
    async fn propose_wazuh_rule(
        &self,
        Parameters(params): Parameters<ProposeWazuhRuleParams>,
    ) -> Result<CallToolResult, McpError> {
        self.custom_tools.propose_wazuh_rule(params).await
    }
    ...
    async fn get_wazuh_vulnerability_summary(
        ...
        self.vulnerability_tools.get_wazuh_vulnerability_summary(params).await
    }
    ...
    async fn get_wazuh_critical_vulnerabilities(
        ...
        self.vulnerability_tools.get_wazuh_critical_vulnerabilities(params).await
    }
